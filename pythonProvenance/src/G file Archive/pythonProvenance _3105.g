...
return_stmt : ref='return' (testlist)? -> ^(RETURN_CMD[$ref] testlist)
            ;
...
import_from: ref='from' NAME 'import'
              (STAR -> ^(IMPORT_CMD[$ref] NAME STAR)
              | import_as_names -> ^(IMPORT_CMD[$ref] NAME import_as_names)
              | LPAREN import_as_names RPAREN -> ^(IMPORT_CMD[$ref] NAME import_as_names)
              )
           ;
...
atom : ref=LPAREN
     ...
     | id1=NAME -> ^(VARIABLE_CMD[$id1] $id1)
     | id1=DOTTED_NAME -> ^(CLASSVARIABLE_CMD[$id1] $id1)
     | id1=INT -> ^(VALUETYPE[$id1] INT_TYPE $id1 )
     | id1=LONGINT -> ^(VALUETYPE[$id1] LONG_TYPE $id1 )
     | id1=FLOAT -> ^(VALUETYPE[$id1] DOUBLE_TYPE $id1 )
