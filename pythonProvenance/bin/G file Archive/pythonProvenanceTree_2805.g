...
funcdef returns [Command result]
	: ^(OUTER_FUNCTION_CMD id1=NAME id2=parameters id3=suite){
		if($id2.result!=null){
			$result = new FunctionDefinitionCommand($id1.text, $id2.result, $id3.result);	
			System.out.println("***FunctionDefinition -> " + $id1.text + " Parameters: "+$id2.result + "Suite: "+$id3.result + " " + $result);
    			commandList.add($result);
    		}
    		else{
    			$result = new FunctionDefinitionCommand($id1.text, $id3.result);	
			System.out.println("***FunctionDefinition -> " + $id1.text + " Suite: "+$id3.result + " " + $result);
    			commandList.add($result);
    		}
	}
        ;
...
import_as_name
	: ^(IMPORT_EQUIV id1=NAME id2=NAME?) {
		if($id2.text!=null)
			c = new ImportEquivalent ($id1.text, $id2.text);
		else
			c = new ImportEquivalent ($id1.text);
		
		commandList.add(c);
		System.out.println("***Import Equivalency " + $id1.text + " as " + $id2.text + " " + c);
...
return_stmt returns [Command result]
	: ^(RETURN_CMD cmd=testlist){
		$result = new ReturnCommand($cmd.result);
		System.out.println("***ReturnCommand "+$cmd.result + " " +$result);
	}
	;
...
if_stmt returns [Command result]
	: ^(IFBLOCK id1=if_clause id2=elif_clauses? id3=else_clause?){
		if($id1.result!=null && $id2.result!=null && $id3.result!=null){
			$result = new IfBlockCommand($id1.result, $id2.result, $id3.result);
			System.out.println("***IfBlockCommand -> "+ $id1.result+ " "+ $id2.result +" "+ $id3.result+" "+$result);
			commandList.add($result);
		}
		else if($id1.result!=null && $id2.result==null && $id3.result!=null){
      $result = new IfBlockCommand($id1.result, $id3.result);
      System.out.println("***IfBlockCommand -> "+ $id1.result+ " "+ $id3.result+" "+$result);
      commandList.add($result);
    }
    else if($id1.result!=null && $id2.result==null && $id3.result==null){
      $result = new IfBlockCommand($id1.result);
      System.out.println("***IfBlockCommand -> "+ $id1.result+ " " +$result);
      commandList.add($result);
    }
    else if($id1.result!=null && $id2.result!=null && $id3.result==null){
      $result = new IfBlockCommand($id1.result, $id2.result);
      System.out.println("***IfBlockCommand -> "+ $id1.result+ " " +$id2.result+ " " +$result);
      commandList.add($result);
    }
    else{
...
if_clause returns [Command result]
	: ^(IF_CMD id1=test id2=suite){
		$result = new IfClauseCommand($id1.result, $id2.result);
		System.out.println("***IfClauseCommand -> " + $id1.result + " "+ $id2.result + " "+$result);
		commandList.add($result);
	}
	;
...
elif_clause
	: ^(ELIF_CMD id1=test id2=suite){
		d = new ElifClauseCommand($id1.result, $id2.result);
		commandList.add(d);
		System.out.println("***ElifClauseCommand -> "+ $id1.result + " " + $id2.result + " "+d);
...
else_clause returns [Command result]
  : ^(ELSE_CMD cmd=suite){
      $result = new ElseClauseCommand($cmd.result);
      System.out.println("***ElseClauseCommand -> " + $cmd.result + " "+$result);
      commandList.add($result);
  }
  ;
...
while_stmt returns [Command result]
  : ^(WHILEBLOCK id1=while_clause id2=else_clause?){
      if($id1.result!=null && $id2.result!=null){
        $result = new WhileBlockCommand($id1.result,$id2.result);
        System.out.println("***WhileBlockCommand -> " + $id1.result + " " + $id2.result + " " + $result);
        commandList.add($result);
      }
      else if ($id1.result!=null && $id2.result==null){
        $result = new WhileBlockCommand($id1.result);
        System.out.println("***WhileBlockCommand -> " + $id1.result + " " + $result);
        commandList.add($result);
      }
      else{
      }
...
while_clause returns [Command result]
  : ^(WHILE_CMD id1=test id2=suite){
      $result = new WhileClauseCommand($id1.result, $id2.result);
      System.out.println("***WhileClauseCommand -> " + $id1.result + " "+ $id2.result + " "+$result);
      commandList.add($result);
  }
  ;
...
try_stmt returns [Command result] 
  : ^(TRYBLOCK id1=try_clause id2=except_clause id3=finally_clause?){
      if($id1.result!=null && $id2.result!=null && $id3.result!=null){
        $result = new TryBlockCommand($id1.result, $id2.result, $id3.result);
        System.out.println("***TryBlockCommand -> " + $id1.result + " " + $id2.result + " " + $id3.result + " "+ $result);
        commandList.add($result);
      }
      else if($id1.result!=null && $id2.result!=null && $id3.result==null){
        $result = new TryBlockCommand($id1.result, $id2.result);
        System.out.println("***TryBlockCommand -> " + $id1.result + " " + $id2.result +" "+ $result);
        commandList.add($result);
      }      
      else{
      }
...
try_clause returns [Command result]
  : ^(TRY_CMD cmd=suite){
      $result = new TryClauseCommand($cmd.result);
      System.out.println("***TryClauseCommand -> " + $cmd.result + " " + $result);
      commandList.add($result);
  }
  ;
...
except_clause returns [Command result]
  : ^(EXCEPT_CMD id1=trailer id2=suite ){
      if($id1.result!=null && $id2.result!=null){
        $result = new ExceptClauseCommand($id1.result, $id2.result);
        System.out.println("***ExceptClauseCommand -> " + $id1.result + " " + $id2.result + " "+$result);
        commandList.add($result);
      }
      else if($id1.result==null && $id2.result!=null){
        $result = new ExceptClauseCommand($id2.result);
        System.out.println("***ExceptClauseCommand -> " + $id2.result + " "+$result);
        commandList.add($result);
      } 
      else{
      }
...
finally_clause returns [Command result]
  : ^(FINALLY_CMD cmd=suite){
      $result = new FinallyClauseCommand($cmd.result);
      System.out.println("***FinallyClauseCommand -> " + $cmd.result + " " +$result);
      commandList.add($result);
  }
  ;
...
power returns [Command result]
	: ^(EXPRESSION id1=atom id2=trailer?){
	  ...
	  if(str.equals("int"))
	   ...
	   $result = new MethodCommand(str, $id2.result);
	   System.out.println("***MethodCommand -> Name: " + str + " Args: " + $id2.result + " " + $result);
...
atom returns [Command result]
	...
	| ^(CLASSVARIABLE_CMD id1=DOTTED_NAME) {
	     $result = new ClassVariableCommand($id1.text);
	     System.out.println("***ClassVariable:"+$id1.text+" "+$result);
       commandList.add($result);
	}
	...
	| ^(VALUETYPE type=STRING_TYPE id1=STRING id2=string_argument?){
		  ...
		  $result = new DatatypeCommand($type.text,$id1.text, $id2.result);
      		  System.out.println("***Value:"+$id1.text+" "+$id2.result + " " + $result);
      		  commandList.add($result);
      		}
	}
...
dictmaker returns [Command result]
    : ^(DICTIONARY_CMD cmd=dict_arguments){
        $result = new DictComprehensionCommand($cmd.result);
        System.out.println("***DictComprehension -> Arguments: " + $cmd.result + " " + $result);
        commandList.add($result);
    }        
    ;
...
classdef returns [Command result]
    : ^(CLASS_CMD id1=NAME id2=suite id3=testlist?){
        if($id3.result!=null){
          $result = new ClassDefinitionCommand($id1.text, $id2.result, $id3.result);
          System.out.println("***ClassDefinitionCommand -> " + $id1.text + " " + $id2.result + " " + $id3.result + " " + $result);
          commandList.add($result);
        }
        else {
          $result = new ClassDefinitionCommand($id1.text, $id2.result);
          System.out.println("***ClassDefinitionCommand -> " + $id1.text + " " + $id2.result + " " + $result);
          commandList.add($result);
        }
    } 
    ;
